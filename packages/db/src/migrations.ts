...
CREATE INDEX IF NOT EXISTS dreams_sleep_session_id_idx ON dreams (sleep_session_id);
CREATE INDEX IF NOT EXISTS dreams_dream_date_idx ON dreams (dream_date);
...
CREATE INDEX IF NOT EXISTS recall_entries_dream_id_idx ON recall_entries (dream_id);
CREATE INDEX IF NOT EXISTS recall_entries_captured_at_idx ON recall_entries (captured_at);
CREATE INDEX IF NOT EXISTS recall_entries_deleted_at_idx ON recall_entries (deleted_at);
CREATE TABLE IF NOT EXISTS recall_audio (
  ...
  FOREIGN KEY (recall_entry_id) REFERENCES recall_entries(id) ON DELETE cascade
);
...
CREATE INDEX IF NOT EXISTS dream_analyses_current_idx ON dream_analyses (dream_id, is_current);
CREATE TABLE IF NOT EXISTS hvdc_records (
  ...
  FOREIGN KEY (analysis_id) REFERENCES dream_analyses(id) ON DELETE cascade
);
...
CREATE INDEX IF NOT EXISTS entities_type_name_idx ON entities (type, name);
...
CREATE INDEX IF NOT EXISTS entity_merges_canonical_idx ON entity_merges (canonical_entity_id);
CREATE INDEX IF NOT EXISTS entity_merges_merged_idx ON entity_merges (merged_entity_id);
...
CREATE INDEX IF NOT EXISTS dream_entities_entity_id_idx ON dream_entities (entity_id);
...
export async function initializeDatabase(options: InitializeDatabaseOptions): Promise<void> {
  const client = createClient(options);
  await client.batch(splitSqlStatements(INITIAL_SCHEMA_SQL).map((sql) => ({ sql, args: [] })));
  client.close();
}
